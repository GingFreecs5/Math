...
\begin{itemize}
	...
	\item \textbf{Limite des fonctions exponentielles et logarithmiques :}
	\[
...
\section{Proposition. Limites usuelles des fonctions en \( +\infty \) et \( -\infty \)}

\subsection{Limites des fonctions polynômes}
...
\subsection{Limites des fonctions rationnelles}
...
\subsection{Limites des fonctions racines carrées}
...
\subsection{Limites des fonctions exponentielles et logarithmiques}
\[
\lim_{x \to +\infty} e^x = +\infty,
\quad \lim_{x \to -\infty} e^x = 0,
\quad \lim_{x \to 0^+} \ln(x) = -\infty,
\quad \text{et} \quad \lim_{x \to +\infty} \ln(x) = +\infty.
\]


\subsection{Limites des fonctions trigonométriques}
\[
\lim_{x \to 0} \sin(x) = 0,
\quad \lim_{x \to +\infty} \sin(x) \text{ n'existe pas},
\quad \lim_{x \to +\infty} \cos(x) \text{ n'existe pas}.
\]
Pour la fonction tangente :
\[
\lim_{x \to \frac{\pi}{2}^-} \tan(x) = +\infty,
\quad \lim_{x \to \frac{\pi}{2}^+} \tan(x) = -\infty.
\]
\section{Démonstration de \(\lim_{x \to 0} \frac{\sin(x)}{x} = 1\)}

\textbf{Démonstration.} Nous utilisons une approche géométrique en considérant le cercle unité. Pour \( x \) en radians, avec \( x > 0 \), on peut montrer les inégalités suivantes :

\[
\cos(x) \leq \frac{\sin(x)}{x} \leq 1.
\]

\textbf{Étape 1 : Construction géométrique.} Considérons un cercle de rayon \(1\) centré à l'origine dans le plan. Le secteur angulaire d'angle \( x \), mesuré en radians, a une aire égale à \( \frac{x}{2} \). En traçant le segment correspondant à \( \sin(x) \) et la tangente \( \tan(x) \), nous obtenons les inégalités suivantes :

\[
\sin(x) \leq x \leq \tan(x).
\]

\textbf{Étape 2 : Normalisation.} En divisant toutes les parties de l'inégalité par \( \sin(x) \) et \( x \), nous obtenons :

\[
1 \leq \frac{x}{\sin(x)} \leq \frac{1}{\cos(x)}.
\]

En inversant cette inégalité, on obtient :

\[
\cos(x) \leq \frac{\sin(x)}{x} \leq 1.
\]

\textbf{Étape 3 : Limite.} Prenons la limite lorsque \( x \) tend vers 0. Nous savons que \( \cos(x) \to 1 \) lorsque \( x \to 0 \), donc, par le théorème des gendarmes (ou théorème d'encadrement), on en déduit que :

\[
\lim_{x \to 0} \frac{\sin(x)}{x} = 1.
\]

\textbf{Conclusion.} Ainsi, nous avons montré que :
