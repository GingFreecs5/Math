...
\begin{document}
	...
	\newpage
	\section{Étude d'une fonction}
	
	L'étude d'une fonction consiste à analyser ses principales propriétés : son domaine de définition, ses variations, les limites aux bornes de son domaine, ainsi que ses asymptotes éventuelles. Voici les étapes à suivre :
	
	\subsection{Domaine de définition}
	
	Il faut commencer par déterminer l'ensemble des valeurs de \( x \) pour lesquelles la fonction est définie. On résout ainsi les équations ou inéquations qui définissent les restrictions de la fonction (comme les racines carrées, les logarithmes ou les dénominateurs de fractions).
	
	\[
	D_f = \{x \in \mathbb{R} \mid f(x) \text{ est définie} \}
	\]
	
	\subsection{Parité de la fonction}
	
	On vérifie si la fonction est paire ou impaire. Cela peut simplifier les calculs ou permettre de déduire le comportement de la fonction sur une partie de son domaine :
	
	\begin{itemize}
		\item \( f \) est \textbf{paire} si \( f(-x) = f(x) \) pour tout \( x \in D_f \).
		\item \( f \) est \textbf{impaire} si \( f(-x) = -f(x) \) pour tout \( x \in D_f \).
	\end{itemize}
	
	\subsection{Limites aux bornes du domaine}
	
	On calcule les limites de la fonction aux bornes de son domaine de définition (en \( +\infty \), \( -\infty \) ou en tout point où la fonction n'est pas définie). Ces limites permettent d'identifier des comportements asymptotiques ou des discontinuités :
	
	\[
	\lim_{x \to a} f(x)
	\]
	où \( a \) est une borne du domaine ou un point particulier.
	
	\subsection{Dérivée de la fonction}
	
	On calcule la dérivée de la fonction \( f \) pour étudier ses variations. La dérivée nous permet de déterminer les intervalles de croissance et de décroissance, ainsi que les extremums locaux (maximums et minimums) :
	
	\[
	f'(x)
	\]
	
	On résout ensuite l'équation \( f'(x) = 0 \) pour trouver les points critiques.
	
	\subsection{Tableau de variations}
	
	À partir du signe de \( f'(x) \), on dresse le tableau de variations de la fonction, qui montre les intervalles où \( f \) est croissante ou décroissante, ainsi que les extremums.
	
	\[
	\text{Tableau de variations}
	\]
	
	\subsection{Étude de la convexité et de la concavité (facultatif)}
	
	On peut aussi calculer	 la dérivée seconde \( f''(x) \) pour étudier la convexité et la concavité de la fonction et déterminer les points d'inflexion.
	
	\subsection{Asymptotes (facultatif)}
	
	Si la fonction présente des limites infinies, on peut déterminer les éventuelles asymptotes horizontales, verticales ou obliques.
	
	\[
	\lim_{x \to +\infty} f(x) = L \quad \text{(Asymptote horizontale)}
	\]
	\[
	\lim_{x \to a^-} f(x) = \pm \infty \quad \text{(Asymptote verticale)}
	\]
	
	\subsection{Courbe représentative}
	
	Après avoir effectué toutes les étapes précédentes, on peut tracer la courbe représentative de la fonction en respectant les informations sur les variations, les extremums, et les asymptotes.
